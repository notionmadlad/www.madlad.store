...
export default function ProductCard({ delay, product }) {
  return (
    <Link href={`/gumroad/${product.gumroad}`}>
      ...
      >
        <div className="w-full flex justify-center group-hover:bg-main-700 transition-all duration-500">
          <Image
            src={product.image}
            width={264}
            height={264}
            alt="logo"
            className="w-1/2 md:w-2/3 xl:w-3/4"
          />
        </div>
        <div className="flex flex-col gap-5 p-6">
          <div className="flex flex-row justify-between font-semibold">
            <p className="text-[20px]">{product.title}</p>
            <p className="text-[16px] px-2.5 py-1 bg-main-700 rounded-[5px]">
              {product.price}
            </p>
          </div>
